...
function Registrar_Pagos_mensuales(btnRegister) {
    ...
    .done(function (resultado) {
      try {
        ...
        if (response.data.length>0) {
          data = response.data;
          const fechasOperacionUnicas = [...new Set(data.map(item => item.fechaoperacion))];
...
function filtrarRegistrosPorFecha(data, fechaSeleccionada,btn,companyValue) {
  ...
  try {
   ...
   console.log(filters);
     //
    fetch('../Controller/export/Controller-print-payment-pdf.php', {
      method: 'POST',
      headers: {
...
function getCompaniIndexedDB() {
  indexedDB.open('miBaseDeDatos').onsuccess = function(event) {
