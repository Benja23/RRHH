    ...
    .done(function (resultado) {
      try {
        ...
        if (response.data.length>0) {
          data = response.data;
          const fechasOperacionUnicas = [...new Set(data.map(item => item.datecreated))];
...
function printPayment(btn){
 ...
 getCompanyIndexedDB().then((companyValue) => {
    if (companyValue) {
      filtrarRegistrosPorFecha(data, fechaSeleccionada,btn,companyValue);
    } else {
      
    }
  })
  .catch((error) => {
    console.error('Error al obtener la empresa desde IndexedDB:', error);
  });
...
function filtrarRegistrosPorFecha(data, fechaSeleccionada,btn,companyValue) {
  try {
    if (!fechaSeleccionada) return;
    // Filtrar registros por fecha
    filters.data = data.filter(function (record) {
      return record.datecreated === fechaSeleccionada;
    });
    
    // Asignar el valor de la compañía
    filters.company = companyValue;

    console.log(filters);
 
   
    fetch('../Controller/export/Controller-payment-extrahours-pdf.php', {
      method: 'POST',
      headers: {
    ...
    .catch(error => console.log('Error al generar o obtener el PDF:', error));
  } catch (error) {
...
function getCompanyIndexedDB() {
  return new Promise((resolve, reject) => {
    var request = indexedDB.open('miBaseDeDatos');
    request.onerror = () => console.log('Error al abrir la base de datos');
    request.onsuccess = (event) => {
      var db = event.target.result;
      var objectStore = db.transaction('empresas').objectStore('empresas');
