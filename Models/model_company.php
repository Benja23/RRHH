...
class Company {
   ...
   function Register_company($id, $name, $description, $address, $phone, $currency, $email, $logo, $flag, $ruc, $branch, $status) {
  ...
  $stmt->bind_param("ssssssssssss", $id, $name, $description, $address, $phone, $currency, $email, $logo, $flag, $ruc, $branch, $status);
    ...
    function Update_company($id, $name, $description, $address, $phone, $currency, $email, $logo, $flag, $ruc, $branch) {
        if ($this->validations->isDuplicate('company', 'name', $name, $id, 'id')) {
            return array('status' => false, 'auth' => true, 'msg' => 'Ya existe un registro con el mismo nombre');
        }

        $sql = "UPDATE company SET name = ?, description = ?, address = ?, phone = ?, currency = ?, email = ?, logo = ?, flag = ?, ruc = ?, branch = ?, updated_at = NOW() WHERE id = ?";
        $stmt = $this->conexion->conexion->prepare($sql);
        $stmt->bind_param("sssssssssss", $name, $description, $address, $phone, $currency, $email, $logo, $flag, $ruc, $branch, $id);
